...
1. The paragraph renders the request box with data attributes (page id, paragraph id, viewed product, cart service URL, labels). Assets are served from embedded resources at `/truvio/buying-assistant/assets/`. Quantity edits and removed lines in the rendered proposal go to `POST /truvio/buying-assistant/reprice`, which prices the remaining lines through the same `ProposalPricer` the engine uses (no model call) and returns the new line prices, tier labels, stock labels and total; the edits are also sent with the next follow-up so the model builds on the edited list.
